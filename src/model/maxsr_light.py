    ...
    def forward(self, x):
        ...
        if x.shape[2] > 2048:
            x = nn.functional.interpolate(
                x, size=(2048, 2048), mode="bilinear", align_corners=False
            )
...
class MaxSRLight(nn.Module):
    def __init__(self, config):
        super(MaxSRLight, self).__init__()
        self.patch_size = config["patch_size"]
        self.embed_dim = config["emb_size"]
        self.num_heads = config["num_heads"]
        self.mlp_dim = config["dim"]
        self.upscale_factor = config["scale_factor"]
        ...
        self.reconstruction = ReconstructionBlock(
            self.embed_dim, self.num_patches, self.patch_size, 3, self.upscale_factor
        )
