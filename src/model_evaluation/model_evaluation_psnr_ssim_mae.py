...
def simulate_evaluation(hr_images, scales):
    ...
    for i, hr_image in enumerate(hr_images):
        for scale in scales:
            ...
            psnr_value, ssim_value, mae_value = calculate_metrics(hr_image, upscaled_image)
            results.append({
                "Scale": f"x{scale}",
                "Image": f"Image {i}",
                "PSNR (dB)": psnr_value,
...
def highlight_max_min(data):
    ...
    if data.ndim == 1:  # Series
        ...
        return [max_attr if v else min_attr if u else '' for v, u in zip(is_max, is_min)]
    else:  # DataFrame
        is_max = data == data.max(axis=0)
        is_min = data == data.min(axis=0)
        return pd.DataFrame(np.where(is_max, max_attr, np.where(is_min, min_attr, '')), index=data.index, columns=data.columns)
...
def display_results(results_df):
    ...
    averages = results_df.groupby('Scale')[['PSNR (dB)', 'SSIM', 'MAE']].mean().reset_index()
    ...
    def apply_highlight(group):
        return group.style.apply(highlight_max_min, subset=['PSNR (dB)', 'SSIM', 'MAE'], axis=0)
